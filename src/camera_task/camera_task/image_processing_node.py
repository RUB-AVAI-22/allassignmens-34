...
class ImageProcessingNode(Node):
    ...
    def __init__(self, cone_detection, edge_tpu, debug):
        ...
        if cone_detection:
            ...
            if edge_tpu:
                self.model = DetectMultiBackend(os.path.dirname(__file__) + '/../models/best-uint8_edgetpu.tflite')
            else:
                self.model = DetectMultiBackend(os.path.dirname(__file__) + '/../models/best-fp16.tflite')
    ...
    def callback(self, msg):
        ...
        if self.cone_detection:
            prediction = image_to_prediction(original_image)

            bbox_msg = prediction_to_bounding_box_msg(prediction)
...
def image_to_prediction(self, original_image):
    if self.cone_detection:
        ...
        prepared_image = prepare_image_for_model(original_image)

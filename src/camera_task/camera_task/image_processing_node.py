...
class ImgProcessingNode(Node):
    ...
    def callback(self, msg):
        ...
        prepared_image = torch.from_numpy(cv2.dnn.blobFromImage(cv2.cvtColor(original_image, cv2.COLOR_RGB2BRG), 1, (self.targetWidth, self.targetHeight), swapRB=True, crop=False).astype(np.uint8)).to(self.model.device)

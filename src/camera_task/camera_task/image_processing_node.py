...
class ImgProcessingNode(Node):
    ...
    def callback(self, msg):
        ...
        prepared_image = torch.from_numpy(cv2.dnn.blobFromImage(cv2.cvt(original_image, cv2.COLOR_RGB2BGR), 1, (self.targetWidth, self.targetHeight), crop=False).astype(np.uint8)).to(self.model.device)
